...
//! Telemetry module
//!
//! This module encompasses telemetry for `varro`.
...
pub fn register_shutdown() {
    ctrlc::set_handler(move || {
        println!();
        tracing::info!(target: "varro", "shutting down...");
        std::process::exit(0);
    })
...
pub fn init(verbose: bool) -> Result<()> {
    let subscriber = match verbose {
        true => get_subscriber("varro=debug".into()),
        false => get_subscriber("varro=info".into()),
    };
