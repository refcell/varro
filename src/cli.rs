...
impl TryFrom<Cli> for Config {
    ...
    fn try_from(cli: Cli) -> Result<Self, Self::Error> {
        let l1_client_rpc_url =
            cli.l1_client_rpc_url.unwrap_or(extract_env!("L1_RPC_URL"));
        let rollup_node_rpc_url = cli
            .rollup_node_rpc_url
            .unwrap_or(extract_env!("ROLLUP_NODE_RPC_URL"));
        let output_oracle_address = cli
            .output_oracle_address
            .unwrap_or(extract_env!("OUTPUT_ORACLE_ADDRESS"));
        ...
        let output_private_key = cli
            .output_private_key
            .unwrap_or_else(|| extract_env!("OUTPUT_PRIVATE_KEY"));
        ...
        let config = Config {
            l1_client_rpc_url,
            rollup_node_rpc_url,
            output_oracle_address: Address::from_str(&output_oracle_address).map_err(
                |_| ConfigError::InvalidOutputOracleAddress(output_oracle_address),
            )?,
            polling_interval,
            num_confirmation,
        ...
        let mut file = File::create(&config_path).map_err(|_| {
            ConfigError::TomlFileCreation(config_path.to_string_lossy().to_string())
        })?;
        file.write_all(
            toml::to_string(&config)
                .map_err(|_| ConfigError::ConfigTomlConversion)?
                .as_bytes(),
        )
        .map_err(|_| ConfigError::TomlFileWrite)?;
